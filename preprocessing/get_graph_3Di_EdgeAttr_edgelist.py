...
def run(file, d1, d3):
    print(file)
    graph_file_path=os.path.join(d3, file)
    graph_edgelist_df=pd.read_csv(graph_file_path,header=None)
    pdb_path=os.path.join(d1,file.replace('_edgelistWithWeight.csv', ''))
    ...
    df_final.to_csv(os.path.join(d3, file.replace('_edgelistWithWeight.csv','_edgelistWithWeight_3Di.csv')), index=False, header=False)
...
files=list(filter(lambda x: x.endswith('.pdb_edgelistWithWeight.csv'), os.listdir(d3)))
# for file in files:
#     run(file, d1, d3)

random.shuffle(files)
pool=mp.Pool(processes=None)
results=[pool.apply_async(run, args = (file,d1,d3,)) for file in files]
