###    original code: https://github.com/RexYing/gnn-model-explainer/blob/bc984829f4f4829e93c760e9bbdc8e73f96e2cc1/explainer/explain.py#L582
###    explain.py
###    Implementation of the explainer.
##     Modified for MIF, by using the "adj" and "adj" related variables as edge index, edge masks in MIF
...
def build_optimizer(opt, lr, params, opt_scheduler='none', weight_decay=0.0):
...
class Explainer:
    def __init__(
        ...
        self.connections = self.adj
        self.edge_mask0 = edge_mask0
        self.label = label
        self.pred = pred
        self.train_idx = train_idx
        self.n_hops = n_hops #customized
    ...
    def explain(
        ...
        if graph_mode:
            ...
            sub_edge_attr = self.edge_attr[graph_idx]
            sub_edge_mask0 = self.edge_mask0[graph_idx]
        ...
        sub_edge_attr = np.expand_dims(sub_edge_attr, axis=0)
        sub_edge_mask0 = np.expand_dims(sub_edge_mask0, axis=0)

        adj   = torch.tensor(sub_adj, dtype=torch.float)
        x     = torch.tensor(sub_feat, requires_grad=True, dtype=torch.float)
        edge_attr   = torch.tensor(sub_edge_attr, dtype=torch.float)
        edge_mask0 = torch.tensor(sub_edge_mask0, dtype=torch.float)
        #label = torch.tensor(sub_label, dtype=torch.long)
        label = sub_label.clone().detach()

        if self.graph_mode:
            #pred_label = np.argmax(self.pred[0][graph_idx], axis=0)
            pred_label = 1 if self.pred[0][graph_idx] > 0.5 else 0
        ...
        self.model.eval()
        #print('explainer.mask (0)', torch.isnan(explainer.mask).sum()) ##debug
        ...
        if model == "grad":
            ...
            print("no grad")
           #print('explainer.mask (1)', torch.isnan(explainer.mask).sum()) ##debug
            explainer.train()
            #print('explainer.mask (2)', torch.isnan(explainer.mask).sum()) ##debug
            begin_time = time.time()
            epoch_losses=[]
            #print('explainer.mask', torch.isnan(explainer.mask).sum()) ##debug
            for epoch in range(self.num_epochs):
                #print('explainer.mask (kk)', torch.isnan(explainer.mask).sum()) ##debug
                explainer.zero_grad()
                explainer.optimizer.zero_grad()
                #print('Ddebug 0 :')
                ypred, adj_atts = explainer(node_idx_new, unconstrained=unconstrained)
                #print('Ddebug 1 :')
                #print('ypred: {}, pred_label: {}, node_idx_new: {}, epoch: {}.'.format(ypred, pred_label, node_idx_new, epoch))
                loss = explainer.loss(ypred, pred_label, node_idx_new, epoch)
                #print('loss: ', loss)
                #print('Ddebug 2 :')
                #print('explainer.mask (kk2)', torch.isnan(explainer.mask).sum()) ##debug
                # with autograd.detect_anomaly():
                loss.backward()
                #print('explainer.mask (kk3)',explainer.mask.flatten()) ##debug
                epoch_losses.append(list(loss.detach().numpy())[0])
                
                explainer.optimizer.step()
                if explainer.scheduler is not None:
                    explainer.scheduler.step()

                #print('explainer.mask (kk4)',explainer.mask.flatten()) ##debug
                #print('explainer.mask:', explainer.mask) ## debug
...
#                         )
                    ...
                    if epoch == 0:
                        ...
                        if self.gpu:
                            node_adj_att=node_adj_att.cuda()
#                             io_utils.log_matrix(
...
#                 )
                #Customized
                #print('explainer.mask[0]', explainer.mask[0].shape)
                masked_adj = (
                    torch.sigmoid(explainer.mask[0].flatten(start_dim=-2)).cpu().detach().numpy()# * sub_adj.squeeze()
                )
            else:
...
#             print("Saved adjacency matrix to ", fname)
        if model == "grad":
            ...
            return masked_adj, epoch_losses
    ...
    def explain_graphs(self, graph_indices):
        ...
        for graph_idx in graph_indices:
            masked_adj, epoch_losses = self.explain(node_idx=0, graph_idx=graph_idx, graph_mode=True)
...
#         io_utils.plot_cmap_tb(self.writer, "tab20", 20, "tab20_cmap")
        ...
        return masked_adjs, epoch_losses
...
#             adj, x, label = adj.cuda(), x.cuda(), label.cuda()
        ## Customized
        connections = torch.tensor(self.adj, dtype=torch.float)
        x = torch.tensor(self.feat, requires_grad=True, dtype=torch.float)
        edge_attr = torch.tensor(self.edge_attr, dtype=torch.float)
        src = torch.tensor(self.src, dtype=torch.long)
        label = torch.tensor(self.label, dtype=torch.long)
        edge_mask0 = torch.tensor(self.edge_mask0, dtype=torch.float)
        if self.gpu:
            connections, x, label, edge_attr, src, edge_mask0 = connections.cuda(), x.cuda(), label.cuda(), edge_attr.cuda(), src.cuda(), edge_mask0.cuda()

        #preds, _ = self.model(x, adj)
        preds, _ = self.model(src, x, edge_attr, connections, edge_mask0) ## customized
        preds.retain_grad()
...
class ExplainModule(nn.Module):
    def __init__(
        ...
        self.scheduler, self.optimizer = build_optimizer(opt, lr, params)
...
#         }
        ...
        #print('self.mask in init (2):', torch.isnan(self.mask).sum()) #debug
    ...
    def construct_edge_mask(self, num_nodes, init_strategy="normal", const_val=0.0): #, const_val=1.0
        #mask = nn.Parameter(torch.FloatTensor(num_nodes, num_nodes))
        mask = nn.Parameter(torch.FloatTensor(1,num_nodes, 30,1)) ## customized.
        if init_strategy == "normal":
...
#             )
            ...
            with torch.no_grad():
                #mask.normal_(1.0, std)
                mask.normal_(0.0, std) # Wenhao, customized
                # mask.clamp_(0.0, 1.0)
        ...
        if self.use_mask_bias:
            ...
            mask_bias = nn.Parameter(torch.FloatTensor(num_nodes, 30)) ## customized
            nn.init.constant_(mask_bias, 0.0)
        else:
            mask_bias = None

        print('mask in construct_edge_mask(): ', torch.isnan(mask).sum())
    ...
    def forward(self, node_idx, unconstrained=False, mask_features=True, marginalize=False):
        ...
        src = self.src.cuda() if self.gpu else self.src
        edge_attr = self.edge_attr.cuda() if self.gpu else self.edge_attr
        connections = self.adj.cuda() if self.gpu else self.adj
        #print('self.mask in ExplainMoudle (1): ', torch.isnan(self.mask).sum())
#         if unconstrained:
...
#                     x = x * feat_mask
        ...
        #print('x: ',x)
        #print('self.mask in ExplainMoudle (2): ', torch.isnan(self.mask).sum())
        ypred, adj_att = self.model(src, x, edge_attr, connections, self.mask) ## customized
        #print('ypred xx: ',ypred)
        if self.graph_mode:
            #print('Inside Debug 0')
            #res = nn.Softmax(dim=0)(ypred[0])
            res = ypred[0] #customized
            #print('Inside Debug 1')
        else:
            node_pred = ypred[self.graph_idx, node_idx, :]
            res = nn.Softmax(dim=0)(node_pred)
        #print('Inside Debug 2')
        return res, adj_att
    ...
    def adj_feat_grad(self, node_idx, pred_label_node):
        ...
        if self.gpu:
            adj = self.adj.cuda()
            connections = self.adj.cuda()
            x = self.x.cuda()
            label = self.label.cuda()
            src = self.src.cuda()
            edge_attr = self.edge_attr.cuda()
            edge_mask0 = self.edge_mask0.cuda()
        else:
            x, adj, edge_attr, connections, src, edge_mask0 = self.x, self.adj, self.edge_attr, self.adj, self.src, self.edge_mask0
        #ypred, _ = self.model(x, adj)
        ypred, _ = self.model(src, x, edge_attr, connections, edge_mask0) ## customized
        if self.graph_mode:
    ...
    def loss(self, pred, pred_label, node_idx, epoch):
        ...
        print('pred_label: ', pred_label) #debug
        if mi_obj:
            ...
            #print("debug 2")
            logit = pred#[gt_label_node] ##Customized
            #print("logit: ",logit)
            #pred_loss = -torch.log(logit)
            if pred_label>0:
                pred_loss = -torch.log(logit + self.EPS) #customized
            else:
                pred_loss = -torch.log((1-logit) + self.EPS) #customized
            #print("debug 3")
        # size
        mask = self.mask
        if self.mask_act == "sigmoid":
        ...
        #print('torch.sum(torch.abs(mask)): ', torch.sum(torch.abs(mask)))
        #print('self.mask: ', self.mask.flatten())
        #size_loss = self.coeffs["size"] * torch.sum(mask) 
        print(mask.shape)
        size_loss = self.coeffs["size"] * torch.sum(mask)/mask.shape[1] #customized
        #size_loss = self.coeffs["size"] * torch.sum(torch.abs(mask)) #Customized
        #print('size_loss: ',size_loss, ", edge_mask shape: ",mask.shape)
...
#         feat_size_loss = self.coeffs["feat_size"] * torch.mean(feat_mask)
        ...
        print('mask: ', mask.flatten())
        #print("-mask * torch.log(mask+self.EPS) ", -mask * torch.log(mask+self.EPS))
        #print("(1 - mask) * torch.log(1 - mask+self.EPS)", (1 - mask) * torch.log(1 - mask+self.EPS))
        mask_ent = -mask * torch.log(mask+self.EPS) - (1 - mask) * torch.log(1 - mask+self.EPS) #customized
        #mask_ent = -self.mask * torch.log(self.mask+self.EPS) - (1 - self.mask) * torch.log(1 - self.mask+self.EPS) #customized
...
#         feat_mask_ent_loss = self.coeffs["feat_ent"] * torch.mean(feat_mask_ent)
        ...
        print('pred_loss: {}, size_loss: {}, lap_loss: {}, mask_ent_loss: {}.'.format(pred_loss,size_loss,lap_loss,mask_ent_loss))
        loss = pred_loss + size_loss + lap_loss + mask_ent_loss
        #loss = pred_loss + lap_loss + mask_ent_loss #wenhao, customizied. Remove size_loss.
        if self.writer is not None:
